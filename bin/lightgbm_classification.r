...
list.of.packages <- c("easypackages", "lightgbm", "formula.tools", "dplyr", "pastecs", "caret", "stringr")
...
patients_data <- as.matrix(patients_data)
...
for(exe_i in 1:execution_number)
{
    ...
    dtrain <- lgb.Dataset(prc_data_train %>% as.matrix(), label = prc_data_train_labels)
    dtest <- lgb.Dataset.create.valid(dtrain, data = prc_data_test %>% as.matrix(), label = prc_data_test_labels)
    ...
    pred <- predict(model, prc_data_test, type="class")

    prc_data_test_pred_bin <- as.numeric(pred)
