...
GSE_code <- "GSE31519" # this line will change for each different dataset
thisGEOplatform <- "GPL96" # this line will change for each different dataset
datasetName <-  "Karn2011" # this line will change for each different dataset
...
LABEL_DETECTED <- TRUE  
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    label_list <- c()
    i <- 1
    for(thisTitle in gset@phenoData@data$"event (1:ch1") { # this line will change for each different dataset
      
	if(grepl("yes, 0: no): 0", thisTitle)) { # this line will change for each different dataset
	      label_list[[i]] <-  1
          } else if(grepl("yes, 0: no): 1", thisTitle)) { # this line will change for each different dataset
	    label_list[[i]] <- 0
	  } else {
             label_list[[i]] <- -1
	  }
