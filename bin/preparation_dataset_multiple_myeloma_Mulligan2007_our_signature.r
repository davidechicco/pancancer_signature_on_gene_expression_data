...
GSE_code <- "GSE9782" # this line will change for each different dataset
thisGEOplatform <- "GPL96" # this line will change for each different dataset
datasetName <-  "Mulligan2007" # this line will change for each different dataset
...
LABEL_DETECTED <- FALSE  
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    label_list <- c()
    i <- 1
    for(thisTitle in gset@phenoData@data$"characteristics_ch1.11") { # this line will change for each different dataset  
      
	if(grepl("Did_Patient_Die(0=No,1=Yes) = 1", thisTitle, fixed=TRUE)) { # this line will change for each different dataset
	      label_list[[i]] <-  0
