...
GSE_code <- "GSE28571" # this line will change for each different dataset
thisGEOplatform <- "GPL570" # this line will change for each different dataset
datasetName <-  "Micke2011" # this line will change for each different dataset
...
LABEL_DETECTED <- FALSE  
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    label_list <- c()
    i <- 1
    for(thisTitle in gset@phenoData@data$"characteristics_ch1.2") { # this line will change for each different dataset
      
	if(grepl("survival: short", thisTitle, fixed=TRUE)) { # this line will change for each different dataset
	      label_list[[i]] <-  0
          } else if(grepl("survival: long", thisTitle, fixed=TRUE)) { # this line will change for each different dataset
	    label_list[[i]] <- 1
	  } else {
             label_list[[i]] <- -1
	  }
