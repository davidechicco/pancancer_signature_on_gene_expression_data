...
GSE_code <- "GSE34771" # this line will change for each different dataset
thisGEOplatform <- "GPL570" # this line will change for each different dataset
datasetName <-  "Kawaguchi2012" # this line will change for each different dataset
...
LABEL_DETECTED <- FALSE  
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    label_list <- c()
    i <- 1
    for(thisTitle in gset@phenoData@data$"outcome:ch1") { # this line will change for each different dataset  
      
	if(grepl("alive", thisTitle, fixed=TRUE) | grepl("survival", thisTitle, fixed=TRUE)) { # this line will change for each different dataset
	      label_list[[i]] <-  1
          } else if(grepl("dead", thisTitle, fixed=TRUE)) { # this line will change for each different dataset
	    label_list[[i]] <- 0
	  } else {
             label_list[[i]] <- -1
	  }
