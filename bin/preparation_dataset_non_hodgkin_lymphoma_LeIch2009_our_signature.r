...
GSE_code <- "GSE16131" # this line will change for each different dataset
thisGEOplatform <- "GPL96" # this line will change for each different dataset
datasetName <-  "Leich2009" # this line will change for each different dataset
...
# print(table(gset@phenoData@data$"characteristics_ch1.3"))
...
LABEL_DETECTED <- TRUE
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    label_list <- c()
    i <- 1
    for(thisTitle in gset@phenoData@data$"characteristics_ch1.3") { # this line will change for each different dataset
