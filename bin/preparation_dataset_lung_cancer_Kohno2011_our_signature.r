...
GSE_code <- "GSE31210" # this line will change for each different dataset
thisGEOplatform <- "GPL570" # this line will change for each different dataset
datasetName <-  "Kohno2011" # this line will change for each different dataset
...
LABEL_DETECTED <- TRUE  
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    label_list <- c()
    i <- 1
    for(thisTitle in gset@phenoData@data$"death:ch1") { # this line will change for each different dataset
      
	if(grepl("dead", thisTitle, fixed=TRUE)) { # this line will change for each different dataset
	      label_list[[i]] <-  0
