...
GSE_code <- "GSE31684" # this line will change for each different dataset
thisGEOplatform <- "GPL570" # this line will change for each different dataset
datasetName <-  "Reister2012" # this line will change for each different dataset
cancer_type <- "bladder_cancer" # this line will change for each different dataset
...
print(table(gset@phenoData@data$"last known status:ch1"))

print(table(gset@phenoData@data$"characteristics_ch1.19"))
...
LABEL_DETECTED <- TRUE
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    label_list <- c()
    i <- 1
    for(thisTitle in gset@phenoData@data$"last known status:ch1") { # this line will change for each different dataset
      
	if(grepl("DOD", thisTitle)) { # this line will change for each different dataset
	      label_list[[i]] <-  0
	 } else if(grepl("NED", thisTitle)) { # this line will change for each different dataset
	    label_list[[i]] <- 1
	  }
	  else {
             label_list[[i]] <- -1
