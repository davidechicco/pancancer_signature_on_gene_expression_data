...
GSE_code <- "GSE45255" # this line will change for each different dataset
thisGEOplatform <- "GPL96" # this line will change for each different dataset
datasetName <-  "Miller2013" # this line will change for each different dataset
...
LABEL_DETECTED <- TRUE  
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    label_list <- c()
    i <- 1
    for(thisTitle in gset@phenoData@data$"dss event (defined as death from breast cancer):ch1") { # this line will change for each different dataset
