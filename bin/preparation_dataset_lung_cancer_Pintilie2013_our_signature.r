...
GSE_code <- "GSE50081" # this line will change for each different dataset
thisGEOplatform <- "GPL570" # this line will change for each different dataset
datasetName <-  "Pintilie2013" # this line will change for each different dataset
...
LABEL_DETECTED <- TRUE
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    label_list <- c()
    i <- 1
    for(thisTitle in gset@phenoData@data$"status:ch1") { # this line will change for each different dataset
      
	if(grepl("alive", thisTitle)) { # this line will change for each different dataset
	      label_list[[i]] <-  1
          } else if(grepl("dead", thisTitle)) { # this line will change for each different dataset
	    label_list[[i]] <- 0
	  } else {
             label_list[[i]] <- -1
	  }
