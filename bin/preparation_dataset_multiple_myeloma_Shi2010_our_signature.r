...
GSE_code <- "GSE24080" # this line will change for each different dataset
thisGEOplatform <- "GPL570" # this line will change for each different dataset
datasetName <-  "Shi2010" # this line will change for each different dataset
cancer_type <- "multiple_myeloma" # this line will change for each different dataset
...
LABEL_DETECTED <- TRUE
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    label_list <- c()
    i <- 1
    for(thisTitle in gset@phenoData@data$"os milestone outcome (24 months):ch1") { # this line will change for each different dataset
      
	if(grepl("1= deceased by 24 months, 0= alive at 24 months: 0", thisTitle)) { # this line will change for each different dataset
	      label_list[[i]] <-  1
	 } else if(grepl("1= deceased by 24 months, 0= alive at 24 months: 1", thisTitle)) { # this line will change for each different dataset
	    label_list[[i]] <- 0
	  }
	    i <- i + 1
        }
