...
GSE_code <- "GSE88770" # this line will change for each different dataset
thisGEOplatform <- "GPL570" # this line will change for each different dataset
datasetName <-  "MetzgerFilho2016" # this line will change for each different dataset
...
LABEL_DETECTED <- TRUE  
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    label_list <- c()
    i <- 1
    for(thisTitle in gset@phenoData@data$"death_disease_specific:ch1") { # this line will change for each different dataset
      
	if(grepl("No", thisTitle)) { # this line will change for each different dataset
	      label_list[[i]] <-  1
          } else if(grepl("Yes", thisTitle)) { # this line will change for each different dataset
	    label_list[[i]] <- 0
	  } else {
             label_list[[i]] <- -1
	  }
