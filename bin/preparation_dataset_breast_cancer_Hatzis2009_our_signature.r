...
GSE_code <- "GSE25066" # this line will change for each different dataset
thisGEOplatform <- "GPL570" # this line will change for each different dataset
datasetName <-  "Hatzis2009" # this line will change for each different dataset
...
print(table(gset@phenoData@data$"characteristics_ch1.13"))
...
LABEL_DETECTED <- TRUE
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    label_list <- c()
    i <- 1
    for(thisTitle in gset@phenoData@data$"characteristics_ch1.12") { # this line will change for each different dataset
      
	if(grepl("drfs_1_event_0_censored: 0", thisTitle)) { # this line will change for each different dataset
	      label_list[[i]] <-  0
	 } else if(grepl("drfs_1_event_0_censored: 1", thisTitle)) { # this line will change for each different dataset
	    label_list[[i]] <- 1
	  }
	  else {
             label_list[[i]] <- -1
