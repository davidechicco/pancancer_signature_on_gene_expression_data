...
GSE_code <- "GSE38832" # this line will change for each different dataset
thisGEOplatform <- "GPL570" # this line will change for each different dataset
datasetName <-  "Beauchamp2014" # this line will change for each different dataset
cancer_type <- "colorectal_cancer" # this line will change for each different dataset
...
LABEL_DETECTED <- FALSE  
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    label_list <- c()
    i <- 1
    for(thisTitle in gset@phenoData@data$"dss_event (disease specific survival):ch1") { # this line will change for each different dataset  
      
	if(grepl("0 (no death)", thisTitle, fixed=TRUE)) { # this line will change for each different dataset
	      label_list[[i]] <-  1
          } else if(grepl("1 (death from cancer)", thisTitle, fixed=TRUE)) { # this line will change for each different dataset
	    label_list[[i]] <- 0
	  } else {
             label_list[[i]] <- -1
	  }
