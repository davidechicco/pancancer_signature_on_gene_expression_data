...
GSE_code <- "GSE65986" # this line will change for each different dataset
thisGEOplatform <- "GPL570" # this line will change for each different dataset
datasetName <-  "Uehara2015" # this line will change for each different dataset
cancer_type <- "ovarian_cancer" # this line will change for each different dataset
...
# print(table(gset@phenoData@data$"prognosis:ch1"))
...
LABEL_DETECTED <- TRUE
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    label_list <- c()
    i <- 1
    for(thisTitle in gset@phenoData@data$"prognosis:ch1") { # this line will change for each different dataset
      
	if(grepl("NED", thisTitle)) { # this line will change for each different dataset
	      label_list[[i]] <-  1
	 } else if(grepl("AWD", thisTitle)) { # this line will change for each different dataset
	    label_list[[i]] <- 1
          } else if(grepl("DOD", thisTitle)) { # this line will change for each different dataset
	    label_list[[i]] <- 0
	  } else {
             label_list[[i]] <- -1
	  }
