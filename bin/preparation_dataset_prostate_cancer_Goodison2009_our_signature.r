...
GSE_code <- "GSE25136" # this line will change for each different dataset
thisGEOplatform <- "GPL96" # this line will change for each different dataset
datasetName <-  "Goodison2009" # this line will change for each different dataset
cancer_type <- "prostate_cancer" # this line will change for each different dataset
...
LABEL_DETECTED <- FALSE  
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    label_list <- c()
    i <- 1
    for(thisTitle in gset@phenoData@data$"staus dead or alive:ch1") { # this line will change for each different dataset  
