...
GSE_code <- "GSE4922" # this line will change for each different dataset
thisGEOplatform <- "GPL96" # this line will change for each different dataset
datasetName <-  "Ivshina2006" # this line will change for each different dataset
cancer_type <- "breast_cancer" # this line will change for each different dataset
...
print(table(gset@phenoData@data$"DFS EVENT (0=censored; 1=event defined as any type of recurrence (local, regional or distant) or death from breast cancer:ch1"))
...
LABEL_DETECTED <- TRUE
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    label_list <- c()
    i <- 1
    for(thisTitle in gset@phenoData@data$"DFS EVENT (0=censored; 1=event defined as any type of recurrence (local, regional or distant) or death from breast cancer:ch1") { # this line will change for each different dataset
      
	if(grepl("1", thisTitle)) { # this line will change for each different dataset
	      label_list[[i]] <-  1
          } else if(grepl("0", thisTitle)) { # this line will change for each different dataset
	    label_list[[i]] <- 0
	  } else {
             label_list[[i]] <- -1
	  }
