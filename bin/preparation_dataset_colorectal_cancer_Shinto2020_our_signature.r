...
GSE_code <- "GSE143985" # this line will change for each different dataset
thisGEOplatform <- "GPL570" # this line will change for each different dataset
datasetName <-  "Shinto2020" # this line will change for each different dataset
...
# print(table(gset@phenoData@data$"characteristics_ch1"))
...
LABEL_DETECTED <- TRUE
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    label_list <- c()
    i <- 1
    for(thisTitle in gset@phenoData@data$"dfs_event:ch1") { # this line will change for each different dataset
