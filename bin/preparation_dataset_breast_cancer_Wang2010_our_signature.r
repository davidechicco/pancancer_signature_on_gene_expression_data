...
GSE_code <- "GSE19615" # this line will change for each different dataset
thisGEOplatform <- "GPL570" # this line will change for each different dataset
datasetName <-  "Wang2010" # this line will change for each different dataset
...
LABEL_DETECTED <- TRUE  
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    label_list <- c()
    i <- 1
    for(thisTitle in gset@phenoData@data$"distant recur (yn):ch1") { # this line will change for each different dataset
      
	if(grepl("N", thisTitle)) { # this line will change for each different dataset
	      label_list[[i]] <-  1
          } else if(grepl("Y", thisTitle)) { # this line will change for each different dataset
	    label_list[[i]] <- 0
	  } else {
             label_list[[i]] <- -1
	  }
