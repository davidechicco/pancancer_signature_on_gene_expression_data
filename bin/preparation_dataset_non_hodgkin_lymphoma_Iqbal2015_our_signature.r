...
GSE_code <- "GSE58445" # this line will change for each different dataset
thisGEOplatform <- "GPL570" # this line will change for each different dataset
datasetName <-  "Iqbal2015" # this line will change for each different dataset
cancer_type <- "non_hodgkin_lymphoma" # this line will change for each different dataset
...
# print(table(gset@phenoData@data$"characteristics_ch1.5"))
...
LABEL_DETECTED <- TRUE
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    label_list <- c()
    i <- 1
    for(thisTitle in gset@phenoData@data$"characteristics_ch1.5") { # this line will change for each different dataset
      
	if(grepl("clinical info: Follow up status: alive", thisTitle)) { # this line will change for each different dataset
	      label_list[[i]] <-  1
	 } else if(grepl("clinical info: Follow up status: dead", thisTitle)) { # this line will change for each different dataset
	    label_list[[i]] <- 0
	  }
	  else {
             label_list[[i]] <- -1
