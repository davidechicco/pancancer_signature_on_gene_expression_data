...
GSE_code <- "GSE10846" # this line will change for each different dataset
thisGEOplatform <- "GPL570" # this line will change for each different dataset
datasetName <-  "Lenz2008" # this line will change for each different dataset
...
# print(table(gset@phenoData@data$"characteristics_ch1.7"))
...
LABEL_DETECTED <- TRUE
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    label_list <- c()
    i <- 1
    for(thisTitle in gset@phenoData@data$"characteristics_ch1.7") { # this line will change for each different dataset
