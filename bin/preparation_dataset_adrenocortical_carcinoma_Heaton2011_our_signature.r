...
GSE_code <- "GSE33371" # this line will change for each different dataset
thisGEOplatform <- "GPL570" # this line will change for each different dataset
datasetName <-  "Heaton2011" # this line will change for each different dataset
cancer_type <- "adrenocortical_carcinoma" # this line will change for each different dataset
...
LABEL_DETECTED <- TRUE
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    label_list <- c()
    i <- 1
    for(thisTitle in gset@phenoData@data$"dead or alive at last followup:ch1") { # this line will change for each different dataset
      
	if(grepl("dead", thisTitle)) { # this line will change for each different dataset
	      label_list[[i]] <-  0
