...
GSE_code <- "GSE37642" # this line will change for each different dataset
thisGEOplatform <- "GPL96" # this line will change for each different dataset
datasetName <-  "Herold2013" # this line will change for each different dataset
...
table(gset@phenoData@data$"life status:ch1")
...
LABEL_DETECTED <- FALSE  
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    label_list <- c()
    i <- 1
    for(thisTitle in gset@phenoData@data$"life status:ch1") { # this line will change for each different dataset  
