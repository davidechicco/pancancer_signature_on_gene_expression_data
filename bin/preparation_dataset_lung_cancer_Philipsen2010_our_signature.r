...
GSE_code <- "GSE19188" # this line will change for each different dataset
thisGEOplatform <- "GPL570" # this line will change for each different dataset
datasetName <-  "Philipsen2010" # this line will change for each different dataset
cancer_type <- "lung_cancer" # this line will change for each different dataset
...
LABEL_DETECTED <- TRUE  
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    label_list <- c()
    i <- 1
    for(thisTitle in (gset@phenoData@data[gset@phenoData@data$"tissue type:ch1"=="tumor",])$"status:ch1") { # this line will change for each different dataset
      
	if(grepl("deceased", thisTitle, fixed=TRUE)) { # this line will change for each different dataset
	      label_list[[i]] <-  0
          } else if(grepl("alive", thisTitle, fixed=TRUE)) { # this line will change for each different dataset
	    label_list[[i]] <- 1
	  } else {
             label_list[[i]] <- -1
	  }
