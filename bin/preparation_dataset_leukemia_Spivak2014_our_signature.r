...
GSE_code <- "GSE47018" # this line will change for each different dataset
thisGEOplatform <- "GPL96" # this line will change for each different dataset
datasetName <-  "Spivak2014" # this line will change for each different dataset
...
LABEL_DETECTED <- FALSE  
 ...
 if(LABEL_DETECTED == TRUE) {
    ...
    label_list <- c()
    i <- 1
    for(thisTitle in gset@phenoData@data$"survival:ch1") { # this line will change for each different dataset  
      
	if(grepl("living", thisTitle, fixed=TRUE)) { # this line will change for each different dataset
	      label_list[[i]] <-  1
          } else if(grepl("deceased", thisTitle, fixed=TRUE)) { # this line will change for each different dataset
	    label_list[[i]] <- 0
	  } else {
             label_list[[i]] <- -1
	  }
